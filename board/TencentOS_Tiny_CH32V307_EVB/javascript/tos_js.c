...
static size_t strnum(jsval_t value, char *buf, size_t len) {
  ...
  return (size_t) sprintf(buf, fmt, dv);
}
...
struct js *tos_js_vm_create(void *buf, size_t len) {
    struct js * vm = js_create(buf, len);
    // add some tos function
...
jsval_t js_run(struct js *js, const char *buf) {
    return  js_eval(js, buf, ~0U);
}
jsval_t js_eval(struct js *js, const char *buf, size_t len) {
  // printf("EVAL: [%.*s]\n", (int) len, buf);
  jsval_t res = js_mkundef();
  if (len == (size_t) ~0U || len == 0) len = strlen(buf);
  js->consumed = 1;
  js->tok = TOK_ERR;
  js->code = buf;
  js->clen = (jsoff_t) len;
