...
//typedef unsigned long long jsval_t;  // JS value
#include "tos_js_func.h"

struct js *js_create(void *buf, size_t len);         // Create JS instance
struct js *tos_js_vm_create(void *buf, size_t len);  // Create Tos JS instance
jsval_t js_run(struct js *, const char *);  // Execute JS code
