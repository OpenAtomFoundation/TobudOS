...
static void LPUART_ReadNonBlocking(LPUART_Type *base, uint8_t *data, size_t length)
{
    ...
    bool isSevenDataBits =
        (((ctrl & LPUART_CTRL_M7_MASK) ? true : false) ||
         (((!(ctrl & LPUART_CTRL_M_MASK)) ? true : false) && ((ctrl & LPUART_CTRL_PE_MASK) ? true : false)));
#endif
...
status_t LPUART_Init(LPUART_Type *base, const lpuart_config_t *config, uint32_t srcClock_Hz)
{
    ...
    for (osrTemp = 4U; osrTemp <= 32U; osrTemp++)
    {
        ...
        if (tempDiff > (config->baudRate_Bps - (srcClock_Hz / ((uint32_t)osrTemp * ((uint32_t)sbrTemp + 1U)))))
        {
            tempDiff = config->baudRate_Bps - (srcClock_Hz / ((uint32_t)osrTemp * ((uint32_t)sbrTemp + 1U)));
            sbrTemp++;
        }
...
status_t LPUART_SetBaudRate(LPUART_Type *base, uint32_t baudRate_Bps, uint32_t srcClock_Hz)
{
    ...
    for (osrTemp = 4U; osrTemp <= 32U; osrTemp++)
    {
        ...
        /* Select the better value between srb and (sbr + 1) */
        if (tempDiff > (baudRate_Bps - (srcClock_Hz / ((uint32_t)osrTemp * ((uint32_t)sbrTemp + 1U)))))
        {
            tempDiff = baudRate_Bps - (srcClock_Hz / ((uint32_t)osrTemp * ((uint32_t)sbrTemp + 1U)));
            sbrTemp++;
        }
...
/* Get the bytes that available at this moment. */
    ...
    if ((0U != ((uint32_t)kLPUART_TxDataRegEmptyFlag & status)) &&
        (0U != ((uint32_t)kLPUART_TxDataRegEmptyInterruptEnable & enabledInterrupts)))
    {
        if (handle->txState == (uint8_t)kLPUART_TxIdle)
        {
