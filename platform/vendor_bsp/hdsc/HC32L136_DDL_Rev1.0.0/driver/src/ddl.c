...
#define SCB_SHB_BASE				(SCB_BASE +  0x018)
typedef struct
{
	unsigned char  SHP[12U];
}SCB_SHB_TYPE;

#define SCB_SHB                ((SCB_SHB_TYPE       *)     SCB_SHB_BASE ) 

void SYSTICK_NVIC_SetPriority(void)
{
	SCB_SHB->SHP[(((unsigned int)(signed int)(-1)) & 0xFUL)-4UL] = (unsigned char)((((1UL << 4) - 1UL) << (8U - 4)) & (unsigned int)0xFFUL);
}

void SysTick_init(unsigned int ticks)
{
  SysTick->LOAD  = (unsigned int)(ticks - 1UL);                     /* set reload register */
    SYSTICK_NVIC_SetPriority();																				/* set Priority for Systick Interrupt */
  SysTick->VAL   = 0UL;                                             /* Load the SysTick Counter Value */
  SysTick->CTRL  = (1UL << 2) | (1UL << 1) |1;                      /* Enable SysTick IRQ and SysTick Timer */
	//SysTick->CTRL |= 0x00000004;                                                     /* Function successful */
}
//@} // DDL Functions
