...
int HAL_GetDevInfoFromFile(const char *file_name, void *dev_info)
{
...
#endif
#ifdef AUTH_MODE_CERT
    char *   devCrtFileName = NULL;
    char *   devKeyFileName = NULL;
#else
    char *devSecret = NULL;
#endif
    char *   JsonDoc        = NULL;
    ...
    /*Get device info*/
    authMode  = LITE_json_value_of(KEY_AUTH_MODE, JsonDoc);
    productId = LITE_json_value_of(KEY_PRODUCT_ID, JsonDoc);
    devName   = LITE_json_value_of(KEY_DEV_NAME, JsonDoc);
...
#ifdef DEV_DYN_REG_ENABLED
    productSecret = LITE_json_value_of(KEY_PRODUCT_SECRET, JsonDoc);
...
#ifdef AUTH_MODE_CERT
    devCrtFileName = LITE_json_value_of(KEY_DEV_CERT, JsonDoc);
    devKeyFileName = LITE_json_value_of(KEY_DEV_PRIVATE_KEY, JsonDoc);
    ...
    // Log_d("mode:%s, pid:%s, devName:%s, devCrtFileName:%s, devKeyFileName:%s", authMode, productId, devName,
    // devCrtFileName, devKeyFileName);
#else
    devSecret       = LITE_json_value_of(KEY_DEV_SECRET, JsonDoc);
    if (NULL == devSecret) {
...
static int iot_parse_subdevinfo_from_json_file(DeviceInfo *pDevInfo)
{
    ...
    /*Get sub device info*/
    productId = LITE_json_value_of(KEY_SUBDEV_PRODUCT_ID, JsonDoc);
    devName   = LITE_json_value_of(KEY_SUBDEV_NAME, JsonDoc);
