...
#include <stdio.h>
#define utils_printf QCLOUD_LOG_D
...
#define AES_VALIDATE_RET( cond )    \
    UTILS_INTERNAL_VALIDATE_RET( cond, UTILS_ERR_AES_BAD_INPUT_DATA )
#define AES_VALIDATE( cond )        \
    UTILS_INTERNAL_VALIDATE( cond )
...
#if defined(UTILS_PADLOCK_C) &&                      \
    ( defined(UTILS_HAVE_X86) || defined(UTILS_PADLOCK_ALIGN16) )
									...
									uint8_t mode, uint8_t *pKey, uint16_t keybits, uint8_t *iv)
{
	...
	if(UTILS_AES_ENCRYPT == mode){
		...
		if( ret != 0 ){
			QCLOUD_LOG_E("Set encry key err,ret:%d", ret);
		...
		if(outBuffLen < (datalen + UTILS_AES_BLOCK_LEN)){
			QCLOUD_LOG_E("Output buffer should not less than datalen+UTILS_AES_BLOCK_LEN for padding");
			ret = QCLOUD_ERR_FAILURE;
			goto exit;
		}
		...
		if( ret != 0 ){
			QCLOUD_LOG_E("Set dencry key err,ret:%d", ret);
			ret = QCLOUD_ERR_FAILURE;
			goto exit;
		}
	}
	...
	if( ret != 0 ){
		QCLOUD_LOG_E("encryt err,ret:%d",ret);
		ret = QCLOUD_ERR_FAILURE;
		goto exit;
	}else{
...
#endif
#if defined(UTILS_CIPHER_MODE_CTR) || defined(UTILS_CIPHER_MODE_CFB) || \
    defined(UTILS_CIPHER_MODE_OFB)
    size_t offset;
#endif
...
int aes_sample(int verbose)
{
	...
	for (i = 0; i<sLen; i++){
		osal_printf("%02x ", (uint8_t)dataIn[i]);
	}

	if(QCLOUD_ERR_SUCCESS == utils_aes_cbc((uint8_t *)dataIn, sLen, (uint8_t *)dataIn,
	...
	if(QCLOUD_ERR_SUCCESS == utils_aes_cbc((uint8_t *)dataIn, datalen, (uint8_t *)dataIn,
		...
		for (i = 0; i < datalen; i++)
		{
			//osal_printf("%02x ", (uint8_t)dataIn[i]);
			if(dataIn[i] == '\0'){
				QCLOUD_LOG_D("payload len:%d",i);
				break;
			}
			osal_printf("%c", dataIn[i]);
		}
	}
