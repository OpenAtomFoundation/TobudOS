...
- **功能描述**
  ...
  创建一个消息队列（消息就是指针）。消息队列是用来在任务间传递指针信息的机制。pool 由用户传入，pool 是一片大小为 (msg_cnt * sizeof(void *)) 的内存 buffer。
...
- **功能描述**
  ...
  创建一个优先级消息队列（消息就是指针）。优先级消息队列是用来在任务间传递指针信息的机制，与消息队列不同的地方在于，优先级消息队列可以给消息赋予一个优先级，较高优先级（数值上更小）的消息会比较低优先级的消息更快传递到等待任务。pool 由用户传入，pool 是一片大小为 (msg_cnt * sizeof(void *)) 的内存 buffer。
...
- **功能描述**
  ...
  创建一个优先级队列。优先级提供了一种对数据进行基于优先级管理的机制，每次出队的总是队列中优先级最高的元素，可以作为上层数据结构或算法的底层数据容器。pool 由用户传入，pool 是一片大小为 (item_cnt * item_size) 的内存 buffer，mgr_array 也是由用户传入，是一片大小为 TOS_PRIO_Q_MGR_ARRAY_SIZE(item_cnt) 的内存 buffer。
...
- **功能描述**
  ...
  创建一个动态分配的优先级队列。优先级提供了一种对数据进行基于优先级管理的机制，每次出队的总是队列中优先级最高的元素，可以作为上层数据结构或算法的底层数据容器。内部的 pool 池及管理数据池由动态内存分配。
...
- **返回值**
  ...
  K_ERR_OBJ_INVALID prio_q 指向的不是一个合法的优先级队列。
...
- **返回值**
  ...
  K_ERR_TIMER_INVALID_DELAY 非法的 delay 参数（试图将一个一次性定时器的 delay 参数修改为 0）。
...
- **返回值**
  ...
  K_ERR_TIMER_INVALID_PERIOD 非法的 period 参数（试图将一个周期性定时器的 period 参数修改为 0）。
...
- **返回值**
  ...
  K_ERR_OBJ_PTR_NULL stopwatch 是空指针。
...
- **返回值**
  ...
  K_ERR_OBJ_INVALID stopwatch 指向的不是一个合法的计时表。
...
- **功能描述**
  ...
  返回计时表剩余的时间，时间单位为系统滴答数，如果计时已到则返回 0。
...
- **功能描述**
  ...
  返回计时表剩余的时间，时间单位为毫秒，如果计时已到则返回 0。
...
- **参数解释**
  ...
  | [in]   | bit_max    | 位图中可记录的最大的 bit 位长度 |

- 注意

  bitmap_tbl 是由用户传入的一片内存区域，其长度由 TOS_BITMAP_SIZE 计算，对于最大的 bit 位长度为 bit_max 的位图存储表来说，其长度为 TOS_BITMAP_SIZE(bit_max)，那么用户可以定义一个 k_bmtbl_t bitmap_tbl[TOS_BITMAP_SIZE(bit_max)]，bitmap_tbl 作为此接口的第二个入参。

- 返回值**

  K_ERR_NONE 计时表创建成功。

  K_ERR_OBJ_PTR_NULL bitmap 或 bitmap_tbl 是空指针。
...
- **返回值**
  ...
  K_ERR_OBJ_PTR_NULL bitmap 是空指针。

  K_ERR_OBJ_INVALID bitmap 指向的不是一个合法的位图。
...
- **返回值**
  ...
  K_ERR_NONE bit 位设置成功。

  K_ERR_BITMAP_EXCEED bit 超出了 bitmap 可以记录的范围。
